...
function _chart(d3,rootData,width)
{
  ...
  function label(d) {
    return (d.data.type === "directory" ? "📁 " : "📄 ") + d.data.name;
  }

  // 🔑 INITIAL STATE:
  // Root open → its children visible
  // Everything below that collapsed
  if (root.children) {
  ...
  const svg = d3.create("svg")
    .attr("width", width)
    .attr("style", "max-width:100%; height:auto; font:12px system-ui;");
